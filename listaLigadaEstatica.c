...
void exibirLista(LISTA* l) {
    ...
    printf("Lista: \" ");
    while (i != INVALIDO) {
        printf("(Chave: %d, Ant: %d, Próx: %d) ", 
               l->A[i].reg.chave, l->A[i].ant, l->A[i].prox);
        i = l->A[i].prox;
    }
    printf("\"\n");
}
...
bool excluirElemLista(LISTA* l, TIPOCHAVE ch) {
    ...
    if (ant == INVALIDO) {
        ...
        if (l->inicio != INVALIDO)
            l->A[l->inicio].ant = INVALIDO;
    } else { 
        l->A[ant].prox = l->A[i].prox;
        if (l->A[i].prox != INVALIDO)
            l->A[l->A[i].prox].ant = ant;
    }
    devolverNo(l, i);
    return true;
}
...
bool inserirElemListaOrd(LISTA* l, REGISTRO reg) {
    ...
    if (ant == INVALIDO) {
        ...
        l->A[i].ant = ant;
        if (l->A[ant].prox != INVALIDO)
            l->A[l->A[ant].prox].ant = i;
        l->A[ant].prox = i;
    }
    return true;
}
